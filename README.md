...
같은 사이트 안에 들어 있는 두 번째 도구입니다. 보유 재료를 입력하면 부족량을 계산해
**지금 뭘 파밍할지**를 순서대로 알려주고, 그 결과를 체크리스트로 내보내 오버레이에 띄웁니다.
...
키는 `/control` 과 **완전히 같은 것**을 씁니다. 한쪽에 한 번 들어간 브라우저는
`/pal` 도 키 입력 없이 열립니다. 환경변수는 추가로 필요한 게 없습니다.

### 오버레이

트래커는 자체 오버레이를 갖지 않습니다. 계산 결과를 `lists(id='pal')` + `items` 로
sync 하므로, **기존 오버레이가 그대로** `/o/pal` 로 렌더합니다.

- OBS 브라우저 소스를 하나 복제해 URL 만 `/o/pal` 로 바꾸면 끝입니다
- 크기·글자 크기 같은 옵션은 `/control` 에서 이 목록을 골라 조정합니다
- `/control` 에서 이 목록은 **읽기 전용**입니다 (다음 sync 때 덮어써지므로)

`/pal` 의 **할 일** 탭 오른쪽에 실제 방송 화면과 같은 컴포넌트로 그린
미리보기가 있습니다. 재고를 고치면 그 자리에서 바뀝니다.

| 설정 | 뜻 |
|---|---|
| 집중 모드 | 지금 할 것 위주로 몇 줄만 (건설 1 + 파밍 3 + 팰 1). 끄면 표시 항목 수만큼 채웁니다 |
| 총량 표시 | 끄면 `발화 기관 32%` 처럼 비율만 방송에 나갑니다 |
| 자동 갱신 | 재고를 고칠 때마다 오버레이를 다시 밀어넣습니다 |

### 탭

| 탭 | 내용 |
|---|---|
| 할 일 | 진행률 + 다음에 할 일 + 오버레이 미리보기 |
| 재고 | **여기가 핵심.** 획득처별로 묶인 보유량 입력 (절대값 / `+10 +100 +500` 증분) |
| 파밍 | 획득처별 전체 목록과 재료별 진행률 |
| 제작 | 1차 가공 → 제련 → 조립 → 최종 부품 순 제작 트리 |
| 거점 | 거점 ①~④ 건축물 개수 + 팰 배치표, 확보해야 할 팰 |
| 데이터 | 레시피·건축물 재료·`unlock_score` 직접 수정 |

### 우선순위 규칙

1. **자동 획득처를 막고 있는 시설 건설** — 물질 생성기 1대만 지으면 광물 12종이 한꺼번에 풀립니다
2. **재료가 충분해서 지금 지을 수 있는 건축물**
3. **수동 파밍** — 사냥·원정처럼 직접 나가야 하는 것. 자동은 팰만 배치하면 시간이 해결합니다
4. **거점에 배치할 팰 확보** — 재료를 다 모아도 팰이 없으면 거점이 안 돕니다

배합으로 얻는 팰(알파 세크메트)은 잡으라고 하지 않고 **부모 확보(라브라돈·스프라돈)**
를 먼저 띄웁니다.

### DB

```bash
npm run db:setup   # schema.sql → schema-pal.sql 순서로 적용
```

팰월드 테이블은 전부 `pal_` 접두사입니다. 접두사 없는 `lists` / `items` 는 체크리스트 것이라
이름이 겹치면 그쪽이 깨집니다. `pal_*` 는 RLS 정책을 하나도 만들지 않아 **anon 이 읽지도 못합니다**
— 방송에 공개되는 건 sync 로 내보낸 `items.label` 뿐입니다.

시드를 고쳤다면 테스트 픽스처도 다시 떠야 합니다:

```bash
npm run db:setup
npm run test        # lib/pal/__fixtures__/seed.json 과 대조
```

`lib/pal/__fixtures__/design-doc-seed.json` 은 설계도 16장을 그대로 옮긴 **동결 스냅샷**입니다.
계산 엔진의 기준점이라 설계가 바뀌어도 건드리지 않습니다.
...
계산 엔진 단위 테스트 + 설계도 16-2 / 16-3 의 손계산 값 대조(최하위 재료 27종, 중간재 23종)
+ 시드 무결성 + 배치표 검증.
