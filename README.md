...
컨트롤 페이지 오른쪽 **오버레이 옵션** 패널에서 체크하면 URL 이 자동으로 만들어지고,
바로 아래 **OBS URL 복사** 버튼으로 복사할 수 있습니다. 미리보기에도 즉시 반영되므로
실제로 어떻게 보일지 확인한 뒤 복사하면 됩니다. 옵션은 체크리스트마다 따로 기억됩니다.
...
**OBS 소스 크기**(너비·높이)는 URL 파라미터가 아니라 OBS 소스 속성 값입니다. 옵션 패널에서
바꾸면 미리보기 크기가 함께 바뀌어서, 그 크기에서 항목이 몇 개까지 보이는지 그대로 확인할 수 있습니다.
...
- 컨트롤 페이지는 모바일 대응 — 폰으로도 체크할 수 있고, PC 컨트롤 화면에 실시간으로 반영됩니다
- **옵션 끄기 / 미리보기 끄기** 로 편집 영역을 넓게 쓸 수 있습니다 (선택 상태는 기억됩니다)
...
app/control/ControlApp.tsx 컨트롤 UX 전체 (낙관적 업데이트 + 롤백 + 실시간 구독)
app/control/OverlayOptions.tsx 오버레이 옵션 패널 + URL 복사
lib/overlayOptions.ts       옵션 타입 / URL 생성 / localStorage 저장
...
- 키가 틀리면 저장된 키를 지우고 다시 입력받습니다.
- 항목은 200자, 제목은 60자로 잘리고 제어문자는 공백으로 정리됩니다.
...
| 증상 | 원인 |
|---|---|
| 배포 후 "환경변수가 설정되지 않았습니다" | Vercel 에 환경변수 4개를 안 넣었거나 오타. 넣은 뒤 **재배포**해야 반영됩니다 |
| 체크·수정은 되는데 삭제만 반영 안 됨 | `alter table public.items replica identity full;` 누락 → `npm run db:setup` |
| 로컬에서 이유 없이 500 | `npm run dev` 가 여러 개 떠 있는지 확인 (`.next` 를 서로 덮어씁니다) |
| OBS 에 검은 사각형 | `body` 에 배경색이 들어간 것. `globals.css` 의 `body` 는 반드시 `transparent` |
| 글자가 뭉개짐 | OBS 변형 핸들로 늘린 것. 옵션의 글자 크기 + 소스 크기로 조절하세요 |
